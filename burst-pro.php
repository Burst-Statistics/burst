<?php
/**
 * Plugin Name: Burst Pro
 * Plugin URI: https://www.wordpress.org/plugins/burst-statistics
 * Description: Get detailed insights into visitors’ behavior with Burst Pro, the privacy-friendly analytics dashboard from Really Simple Plugins.
 * Version: 1.5.1
...
if ( ! class_exists( 'BURST' ) ) {
	class BURST {
		...
		public static function instance() {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof BURST ) ) {
				...
				if ( burst_admin_logged_in() ) {
					self::$instance->license         = new burst_license();
					...
					/* pro instances */
					self::$instance->pro_statistics = new burst_pro_statistics();
					self::$instance->geo_ip_db      = new burst_geo_ip();
					self::$instance->archive        = new burst_archive();
				}
				self::$instance->hooks();
		...
		private function setup_constants() {
			...
			define( 'burst_plugin_name', 'Burst Pro' );
			...
			define( 'burst_version', '1.5.1' . $debug );
		...
		private function includes() {
			...
			if ( burst_admin_logged_in() ) {
				...
				require_once( burst_path . 'pro/class-licensing.php' );
				...
				require_once( burst_path . 'pro/class-archive.php' );
		...
		private function load_translation() {
			load_plugin_textdomain( 'burst-statistics', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
		}
