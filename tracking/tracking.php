...
if ( ! function_exists( 'burst_track_hit' ) ) {
	...
	function burst_track_hit( $data ): string {
		...
		if ( $sanitized_data['referrer'] === 'spammer' ) {
			burst_error_log( 'Burst Statistics: Referrer spam prevented.' );
		...
		if ($previous_hit !== null) {
			error_log('previous_hit: ' . print_r($previous_hit, true));
			...
			error_log('isDifferentPage: ' . $isDifferentPage);
			error_log('isTimeOverThreshold: ' . $isTimeOverThreshold);
			error_log('isPreviousHitNotBounce: ' . $isPreviousHitNotBounce);
			...
			error_log('previous_hit is null');
		}

		error_log('sanitized_data: ' . print_r($sanitized_data, true));

		$sanitized_data           = apply_filters( 'burst_before_track_hit', $sanitized_data );

		error_log('hit_type: ' . $hit_type);
		$session_arr   = array(
...
if (!function_exists('burst_create_statistic')) {
	...
	function burst_create_statistic($data) {
		...
		if (!burst_required_values_set($data)) {
			burst_error_log('Burst Statistics: Missing required values for statistic creation. Data: ' . print_r($data, true));
			return false;
		}
		...
		if ($inserted) {
			...
			burst_error_log('Burst Statistics: Failed to create statistic. Error: ' . $wpdb->last_error);
			return false;
		}
	}
}
if (!function_exists('burst_update_statistic')) {
	...
	function burst_update_statistic($data) {
		...
		if (!isset($data['ID'])) {
			burst_error_log('Burst Statistics: Missing ID for statistic update. Data: ' . print_r($data, true));
			return false;
		}
		...
		if ($updated === false) {
			burst_error_log('Burst Statistics: Failed to update statistic. Error: ' . $wpdb->last_error);
			return false;
		}
...
if ( ! function_exists( 'burst_create_goal_statistic' ) ) {
	...
	function burst_create_goal_statistic( $data ) {
		...
		if ( ! isset( $data['statistic_id'] ) || ! isset( $data['goal_id'] ) ) {
			...
			burst_error_log( 'Burst Statistics: Goal statistic not created because statistic_id or goal_id is not set' );
		...
		// goal already exists
		if ( $goal_exists ) {
			burst_error_log( 'Burst Statistics: Goal statistic already exists' );
	...
	function burst_is_ip_blocked(): bool {
		...
		if ( is_array( $blocked_ips ) ) {
			...
			if ( in_array( $ip, $ip_blocklist ) ) {
				burst_error_log( 'Burst Statistics: IP ' . $ip . ' is blocked for tracking' );
