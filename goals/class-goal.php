	...
	class burst_goal {
		...
		public function save(): void {
			...
			if ( $this->id > 0 ) {
				...
				$wpdb->update( $table_name, $args, array( 'ID' => $this->id ) );
			} else if ( $this->can_add_goal() ) {
		...
		public function delete(): bool {
			global $wpdb;
			$table_name = $wpdb->prefix . 'burst_goals';
			$result1 = $wpdb->delete( $table_name, array( 'ID' => $this->id ) );

			$table_name_statistics = $wpdb->prefix . 'burst_goal_statistics';
			$result2 = $wpdb->delete( $table_name_statistics, array( 'goal_id' => $this->id ) );
		...
		public function add_predefined(string $id): burst_goal {
			...
			$filtered_goals = array_filter($goals, static function($goal) use ($id){
				return $goal['id'] === $id;
			});
		...
		private function get_available_goal_types(): array {
			...
			$type_field = array_filter($fields, static function($goal) {
