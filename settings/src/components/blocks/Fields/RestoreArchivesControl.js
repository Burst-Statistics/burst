...
const RestoreArchivesControl = () => {
    const { archives, archivesLoaded, fetchData, deleteArchives, downloadUrl, startRestoreArchives, fetchRestoreArchivesProgress, restoring, progress} = useArchiveStore();
    ...
    const onRestoreArchives = async (ids) => {
        ...
        addHelpNotice('archive_data', 'warning', __("Because restoring files can conflict with the archiving functionality, archiving has been disabled.", "burst-statistics"), __("Archiving disabled", 'burst-statistics'))
    }
    ...
    const columns = [
        {
            name: <input type="checkbox" className={indeterminate ? "burst-indeterminate" : ''} checked={entirePageSelected} onChange={(e) => handleSelectEntirePage(e.target.checked)} />,
    ...
    filteredArchives.forEach(archive => {
        ...
        archiveCopy.selectControl = <input type="checkbox" disabled={archiveCopy.restoring || restoring} checked={selectedArchives.includes(archiveCopy.id)}  onChange={(e) => onSelectArchive(e.target.checked ,archiveCopy.id)} />
        ...
        <>
            <div className="burst-table-header">
                <div className="burst-table-header-controls">
                    <input className="burst-datatable-search" type="text" placeholder={__("Search", "burst-statistics")} value={searchValue} onChange={ ( e ) => setSearchValue(e.target.value) } />
                </div>
            </div>

            {
                ...
                <div className="burst-selected-archive">
                    {selectedArchives.length>1 && __("%s items selected", "burst-statistics").replace("%s", selectedArchives.length)}
                    {selectedArchives.length===1 && __("1 item selected", "burst-statistics")}
                    <div className="burst-selected-archive-controls">
                        {showDownloadButton && <>
                            <button disabled={downloading || (progress && progress<100)} className="burst-button burst-button--secondary" onClick={() => downloadArchives()}>
                                {__("Download", "burst-statistics")}
                                {downloading && <Icon name = "loading" color = 'grey' />}
                            </button>
                        </> }
                        <button disabled={progress && progress<100} className="burst-button burst-button--primary" onClick={() => onRestoreArchives(selectedArchives)}>
                            {__("Restore", "burst-statistics")}
                            {progress<100 && <Icon name = "loading" color = 'grey' />}
                        </button>
                        <button disabled={progress && progress<100} className="burst-button burst-button--tertiary" onClick={() => onDeleteArchives(selectedArchives)}>{__("Delete", "burst-statistics")}</button>
                    </div>
                </div>
            }
            {
                ...
                <div className="burst-selected-archive">
                    {__("Restore in progress, %s complete", "burst-statistics").replace('%s', progress+'%')}
                </div>
            }
            ...
            { DataTable && <>
                <DataTable
                    ...
                    noDataComponent={<div className="burst-no-archives">{__("No archives", "burst-statistics")}</div>}
