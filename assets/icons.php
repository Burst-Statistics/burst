 ...
 * @return string
 */
function burst_icon( $icon_name, $status = 'default', $tooltip = '', $size = 15, $copy_id = false, $copy_text = false ) {
    ...
    $icon_html = '<div class="burst-icon burst-' . esc_attr( $status ) . ' burst-icon-' . esc_attr($icon_name) . '" >
					' . $icons[$icon_name] . '
                </div>';

    if ( !empty($tooltip) ) {
        // get help tip
        $icon_html = burst_help_tip( $tooltip, $icon_html );
    }
    if ( !empty($copy_id) && !empty($copy_text) ){
        $icon_html = '<span class="burst-copy-shortcode">
						<span class="burst-copy-shortcode-text" id="' . $copy_id . '">' . $copy_text . '</span>
						' . $icon_html . '
						</span>';
    }
...
function burst_help_tip( $str, $content = false ) {
    $content = !$content ? burst_icon('help') : $content;
    ob_start();
    ?>
    <span class="burst-icon-tooltip" burst-tooltip="<?php echo esc_attr($str) ?>">
			<?php echo $content ?>
		</span>
    <?php
    return ob_get_clean();
