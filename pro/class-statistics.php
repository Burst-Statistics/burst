...
/**
 * Niet in gebruik op dit moment. Maar dit is hoe we de chart JS data voor de experimenten ophaalden.
 */
	...
	class burst_statistics{
		...
		public function ajax_get_experiment_statistics(){
			...
			if (!$error) {
                ...
                $metrics = apply_filters("burst_metrics", array(
                ...
                $options = array();
                $options['scales']['yAxes'][0]['scaleLabel'] = isset($_GET['metric']) && in_array($_GET['metric'], $metrics) ? $metrics[$_GET['metric']] : 'Count';
			}
		...
		public function get_grouped_statistics_array( $args = array() ) {
			global $wpdb;
			$default_args = array(
                'metric' => 'visits', //conversion_percentages, visits, conversions
                'start' => '',
                'end' => '',
                'experiment_id' => '',
                'test_version' => '',
